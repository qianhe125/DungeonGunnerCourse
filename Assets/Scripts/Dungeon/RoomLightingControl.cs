...
public class RoomLightingControl : MonoBehaviour
{
    ...
    private IEnumerator FadeInRoomLightingCoroutine(InstantiatedRoom instantiatedRoom)
    {
        ...
        //如果每进入一个房间需要重新创建一个材质,会浪费显存
        Material litMaterial = GameResources.Instance.litMaterial;
        instantiatedRoom.groundTilemap.GetComponent<TilemapRenderer>().material = litMaterial;
        instantiatedRoom.decoration1Tilemap.GetComponent<TilemapRenderer>().material = litMaterial;
        instantiatedRoom.decoration2Tilemap.GetComponent<TilemapRenderer>().material = litMaterial;
        instantiatedRoom.frontTilemap.GetComponent<TilemapRenderer>().material = litMaterial;
        instantiatedRoom.minimapTilemap.GetComponent<TilemapRenderer>().material = litMaterial;
    }
}